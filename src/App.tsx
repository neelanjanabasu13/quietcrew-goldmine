...
type Result = { place_id: string; name: string; address: string; website?: string | null; rating?: number; review_count?: number; ai?: { visibility?: number; mentions?: number; total?: number } };
...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    {run && <section className="results" aria-live="polite"><div className="run-heading"><div><p className="eyebrow">{run.category} · {run.location}</p><h2>{run.status === "complete" ? "Your prospect list is ready" : run.status === "error" ? "Scan stopped" : "Building your prospect list"}</h2></div>{run.stage_counts?.discovering ? <strong>{run.stage_counts.discovering} found</strong> : null}</div>
      ...
      {results.length > 0 && <div className="table-wrap"><table><thead><tr><th>Business</th><th>Rating</th><th>AI mentions</th><th>Website</th></tr></thead><tbody>{results.map((business) => <tr key={business.place_id}><td><strong>{business.name}</strong><small>{business.address}</small></td><td>{business.rating ? `${business.rating.toFixed(1)} ★` : "—"}<small>{business.review_count || 0} reviews</small></td><td><b className={Number(business.ai?.visibility || 0) < 40 ? "gap" : "visible"}>{business.ai?.visibility != null ? `${business.ai.visibility}%` : "Testing…"}</b><small>{business.ai?.mentions ?? 0} of {business.ai?.total ?? 10} prompts</small></td><td>{business.website ? <a href={business.website} target="_blank" rel="noreferrer">Visit site ↗</a> : "No website"}</td></tr>)}</tbody></table></div>}{run.status === "complete" && results.length === 0 && <p className="empty">No matching businesses were returned for this area. Try a nearby neighbourhood.</p>}
